...
def read_inputs(debug=False):
    ...
    if(debug):
        inputPath = "../../data/processed/coord_input/debug/"
    else:
        inputPath = "../../data/processed/coord_input/"
    
    input_cache_file = np.load(inputPath+"coord_train.npz")
    input_train = input_cache_file["coordinates_train"]
    
    input_cache_file = np.load(inputPath+"coord_test.npz")
...
def neural_network (data_train,
    ...
    history = model.fit(data_train, label_train, epochs=5)
    ...
    tic()
    # classify some data
    salida_de_la_red = model.predict_classes(data_validation, verbose=1)
    tiempo_test_ms = toc()
    
    return salida_de_la_red, tiempo_entrenamiento_ms, tiempo_test_ms

def calularAcuracia(label_val, out_net):
...
def calcularMatrixDeConfusion(labels_de_test, 
                              salida_de_red,
                              titulo):
    ...
    # path_result = "../../results/"
    # np.savetxt('confusionMatrix/'+str(nombre_del_experimento)+'/MC_'+nombre_del_experimento+str(numero_del_experimento)+'_'+str(numero_de_la_matriz) +'.csv', matriz_de_confusion_prueba, delimiter=',', fmt='%d')
...
def select_best_beam(enableDebug = False):
    ...
    for j in range(len(address_size)): #For encargado de variar el tamano de la memoria
        ...
        for i in range(numero_experimentos): #For encargado de ejecutar el numero de rodadas (experimentos)
            ...
            matrizdeconfusion = calcularMatrixDeConfusion(label_validation1,
                                                          out_red,
                                                          titulo)
            ...
            acuracia = calularAcuracia(label_validation1, out_red)
        ...
        df_cm = pd.DataFrame(matriz_confusion_media, index=range(0,numero_de_grupos), columns=range(0,numero_de_grupos))
